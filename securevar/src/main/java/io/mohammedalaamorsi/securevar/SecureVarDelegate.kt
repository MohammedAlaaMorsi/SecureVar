...
class SecureVarDelegate<T>(
    ...
    // Secrets sourced from SecureVarManager's SecretProvider (fallback to static placeholders if absent)
    private val macSecret: String by lazy {
        SecureVarManager.secretProvider?.getMacSecret() ?: "delegate-mac-secret"
    }
    private val encBaseSecret: String by lazy {
        SecureVarManager.secretProvider?.getEncSecret(propertyName) ?: "delegate-enc-secret"
    }
    ...
    private val isBoolean = initialValue is Boolean
    private val isInt = initialValue is Int
    private val isLong = initialValue is Long
    private val isString = initialValue is String
    ...
    override fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
        ...
        SecureVarManager.trigger("tamper.set", "Illegal direct assignment to $propertyName")
    ...
    fun authorizedWrite(newValue: T, key: WriteKey) {
        ...
        if (!allowWriteNow()) {
            SecureVarManager.trigger("tamper.rate", "Write rate exceeded for $propertyName")
            return
        }

        // 1. Validate the key via optional external verifier first (server logic mirrored locally)
        val externalVerifier = SecureVarManager.writeKeyVerifier
        if (externalVerifier != null) {
            if (!externalVerifier.verify(key)) {
                SecureVarManager.trigger("tamper.write", "External verifier rejected key for $propertyName")
                return
            }
            ...
            if (!key.isValid()) {
                SecureVarManager.trigger("tamper.write", "Invalid write key for $propertyName")
                return
            }
        }
    ...
    private fun parseToType(text: String): T {
        return when {
            isBoolean -> (text.toBooleanStrictOrNull() ?: false) as T
            isInt -> (text.toIntOrNull() ?: 0) as T
            isLong -> (text.toLongOrNull() ?: 0L) as T
            isString -> text as T
            else -> text as T
        }
    }
    ...
    private fun getDefaultValue(): T {
        // Provide safe defaults based on initial type
        return try {
